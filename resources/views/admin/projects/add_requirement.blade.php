...
@section('content')
    ...
    <div class="modal-footer" style="place-content: center;">
        <a type="button" href="{{ route('/project/{projectId}/systems.index',$project->id) }}"
           class="btn btn-secondary" data-dismiss="modal">{{ trans('title.back') }}</a>
        <a href="{{ route('project/{projectId}/systems/requirements/subfunctions.index',$project->id) }}"
           class="btn btn-primary">{{ trans('title.next') }}</a>
    </div>
@endsection
...
@section('after_scripts')
    ...
    <script>
        <?php
        $dataArray = $systems->toArray();
        ?>
        let data = @if(!empty($systems)) @json($dataArray['data']) @else [] @endif;
        let csrf = "{{ csrf_token() }}";
        let projectId = {{ $project->id }};
        let url = '/admin/api/project/systems/requirements/';
        @if(isset($systemList))
        $("#addParent").change(function () {
            projectId = $('#addParent').val();
        });
        @endif
    </script>
